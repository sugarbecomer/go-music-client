...
func NewKWSearchApi(pageNo, pageSize int, key string) string {
...
func NewKWApi() KWApi {
	var kw KWApi
	for i := 0; i < 63; i++ {
		kw.ArrayMask[i] = int64(math.Pow(float64(2), float64(i)))
	}
	kw.ArrayMask[len(kw.ArrayMask)-1] = -9223372036854775808
	return kw
}

func (k *KWApi) base64Encrypt(str string) string {
	bs := k.encrypt([]byte(str), []byte(SECRET_KEY))
	str = base64.StdEncoding.EncodeToString(bs)
	return strings.ReplaceAll(str, "\n", "")
}

func (k *KWApi) encrypt(msg, key []byte) []byte {
...
func (k *KWApi) DES64(ls []int64, l int64) int64 {
	var out int64
	var SOut int64
	var pR [8]int64
	var pSource [2]int64
...
func (k *KWApi) subKeys(l int64, ls []int64, n int) []int64 {
...
func (k *KWApi) bitTransform(arrInt []int, n int, l int64) int64 {
	var l2 int64
	for i := 0; i < n; i++ {
...
func (k *KWApi) SearchMusic(kw string) *dto.KWResp {
	api := NewKWSearchApi(0, 10, kw)
...
func (k *KWApi) getTrackUrl(mid, quality string) string {
	switch quality {
	case cst.STAND:
		quality = "128kmp3"
	case cst.HIGHT:
	...
	if r.Data.URL == "" || strings.Contains(r.Data.URL, "/4141006416.mp3") ||
		...
		return k.getTrackUrl3(mid, quality)
	}

	return r.Data.URL
}

func (k *KWApi) getTrackUrl3(mid, quality string) string {
	switch quality {
	case cst.STAND:
